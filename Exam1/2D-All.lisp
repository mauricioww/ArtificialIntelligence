...
(defun valid-operator?2D (op state)
...
"Obtiene todos los descendientes válidos de un estado, aplicando todos los operadores en *ops* en ese mismo órden"
     (let ((descendientes  nil)
		 ...
		 (when (and (valid-operator?2D  op  estado) (valid-state?2D (first new-state) (second new-state)))           ;; se valida el resultado...
...
(defun expandBFS(state)
    (let
        ...
        (dolist (op *operators*)
            ...
            (cond (
                (and (valid-operator?2D op currentC) (valid-state?2D (first new-state) (second new-state)) )   ;; Check if is correct the new transition
...
(defun expand-A-star (state)
  (let*
    ...
    (dolist  (op  *operators*)
      ...
      (cond 
        ( (and (valid-state?2D (first new-coord) (second new-coord)) (valid-operator?2D op (second state)) ) 
